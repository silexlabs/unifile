...
class FsConnector {
   ...
   * @parma {string} [config.rootPath] - Path against all relative paths will be resolved.
  ...
  constructor(config) {
    this.config = config || {};
    this.infos = Tools.mergeInfos(this.config.infos, {
      name: NAME,
      displayName: 'Local',
      icon: '',
    ...
    else if(this.config.sandbox.constructor === String) this.sandbox = [this.config.sandbox];
    else if(Array.isArray(this.config.sandbox)) this.sandbox = this.config.sandbox;
    else throw new Error('Invalid sandbox path. Must be a string or an array');

    if(config && config.rootPath) this.rootPath = config.rootPath;
  ...
  readFile(session, path) {
    try {
      return Fs.readFilePromised(this[validatePath](path), 'utf8');
    } catch (e) {
      return Promise.reject(e);
    }
  }
