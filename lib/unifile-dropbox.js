...
function checkBatchEnd(session, result, checkRoute, jobId){
  switch(result['.tag']){
    case 'async_job_id':
      ...
      // Set ID and fall to the next case
    case 'in_progress':
...
class DropboxConnector {
  ...
  readFile(session, path) {
    if(!session.token)
      ...
      return callAPI(session, '/files/download', data, 'content', false, {
        'Content-Type': 'application/octet-stream',
        'Dropbox-API-Arg': JSON.stringify({
          path: makePathAbsolute(path)
        })
  ...
  batch(session, actions, message) {
    if(!session.token)
      ...
      for(const action of actions){
        switch (action.name.toLowerCase()) {
          ...
          case 'writefile':
            ...
            break;
          case 'mkdir':
            promises.push(mkdir(session, action.path));
            break;
          default:
      ...
      if(uploadEntries.length > 0) {
        promises.push(
          Promise.map(uploadEntries, action => {
          ...
          .then(entries => closeUploadBatchSession(session, entries))
