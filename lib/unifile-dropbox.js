  ...
  batch(session, actions, message) {
    if(!session.token)
      ...
      return Promise.each(actions, (action) => {
        // TODO Use real batch operations
        switch (action.name.toLowerCase()) {
          case 'unlink':
          case 'rmdir':
          case 'mkdir':
            return this[action.name](session, action.path);
          case 'rename':
            return this[action.name](session, action.path, action.destination);
          case 'writefile':
            // TODO Find a way to ignore case in method name
            return this.writeFile(session, action.path, action.content);
        }
      });
    }
  }
