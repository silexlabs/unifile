 ...
 */
function commit(session, repo, tree, message) {
	var apiPath = '/repos/' + session.gh_account.login + '/' + repo + '/git';
	var token = session.gh_token;
	var lastCommitSha;

	return callAPI(token, apiPath + '/refs/heads/master', null, 'GET')
	.then(function(res){
	...
	.then(function (res) {
		...
		return callAPI(token, apiPath + '/refs/heads/master', data, 'PATCH');
	});
}
 ...
 * @param  {string} session.gh_account.login - GitHub login name
 * @param  {Object} session.gh_token - Connection token
...
function move(session, repo, src, dest, branch) {
	...
	.then(function(res) {
		return res.tree.map(function(file) {
			// ^test$|^test(\/)
	...
	if (!isConnected(request)){
		...
		switch (splitPath.length) {
			...
			default: // Create a folder (with a .gitignore file in it because git doesn't track empty folder)
				...
				apiPromise = callAPI(request, apiPath + '/.gitignore', reqData, 'PUT');
		}
...
exports.put = function (path, data, request, response, next, cbk) {
	if (!isConnected(request)){
		...
		if (splitPath.length < 3) {
			return cbk({success: false, message: 'Invalid path. It must at least have 3 levels.'});
		}
		var filePath = splitPath.slice(2).join('/');
		commit(request.session, splitPath[0], [{
			path: filePath,
			content: data,
			mode: '100644',
			type: 'blob'
...
exports.get = function (path, request, response, next, cbk) {
	if (!isConnected(request)){
		...
		callAPI(request.session.gh_token, apiPath, null, 'GET')
		.then(function(res){
			if (res.type === 'file') {
				cbk({success: true, message: new Buffer(res.content, res.encoding).toString()});
			}
			else {
				cbk({success: true, message: res.map(function(sub){return sub.name})});
			}
		})
		.catch(function(err){
			cbk({success: false, message: err});
	...
	if (!isConnected(request)){
		...
		switch (splitPath.length) {
			...
			case 2:	// Remove branch
				resultPromise = callAPI(request.session.gh_token, repoPath + '/git/refs/heads/' + splitPath[1], null, 'DELETE');
				break;
			default: // Remove file/folder
				var filePath = splitPath.slice(2).join('/');
				resultPromise = callAPI(request.session.gh_token, repoPath + '/contents/' + filePath, {ref: splitPath[1]}, 'GET')
				.then(function(res) {
					if(!Array.isArray(res)){
						...
						return callAPI(request.session.gh_token, repoPath + '/contents/' + filePath, reqData, 'DELETE');
					}
				});
		}
		resultPromise
