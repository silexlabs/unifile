...
exports.init = function (app, express, options) {
	...
	app.get(exports.config.auth_form_route, function(request, response, next){
		response.send(exports.config.auth_form_html);
	});
	...
	app.post(exports.config.auth_form_submit_route, function(request, response, next){
		...
		if (request.param("password") && request.param("username")
			...
			request.session.www_user = {
				display_name: request.param("username")
			};
			if (!exports.checkAuth("admin", "admin")){
				exports.config.auth_form_warning = '';
			}
			response.send('<html><head><script type="text/javascript">window.onload=function(){window.close();};</script></head><body><p>close this window please</p>'+exports.config.auth_form_warning+'</body></html>');
		}
		else{
			console.error("Wrong login or password");
			response.send(401, "<html><head></head><body>Wrong login or password for "+request.param("username")+"</body></html>");
		}
	});
}
