...
var dbox  = require("dbox-timecorrection");
...
/**
 * horrible temporary workaround
 * FIXME: find out why the calls to dbox lib sometimes never return
 */
exports.startWatching = function (request, response, next, cbk, methodName) {
  return setTimeout(function () {
    console.error('Error: the method' + methodName + 'never returned. Applying workaround, just return an error.');
    // return an error
    cbk({success:false, code:401, message:"Something went wrong! Logout from dropbox and try again in 1 min."});
  }, 10000);
}
/**
 * horrible temporary workaround
 * FIXME: find out why the calls to dbox lib sometimes never return
 */
exports.stopWatching = function (timer) {
...
exports.connect = function (request, response, next, cbk) {
  exports.logout(request, response, next, function () {
    var timer = exports.startWatching(request, response, next, cbk, 'dboxapp.requesttoken');
    exports.dboxapp.requesttoken(function(status, request_token){
      exports.stopWatching(timer);
////////////////////////
...
exports.login = function (request, response, next, cbk) {
  if (!request.session.dropbox_request_token){
    ...
    if (request.session.dropbox_access_token){
      ...
      var timer = exports.startWatching(request, response, next, cbk, 'dboxapp.accesstoken');
      exports.dboxapp.accesstoken(request.session.dropbox_request_token, function(status, access_token){
        exports.stopWatching(timer);
        if (status!==200){
          cbk(
            {success:false}
...
exports.get = function (path, request, response, next, cbk) {
	if (!request.session.dropbox_access_token){
		...
		exports.getClient(request).get(path, function(status, reply, metadata){
			if (status !== 200){
        var msg = 'Could not get the file ' + path;
        if (reply){
          msg = reply.toString();
        }
				cbk({success:false, code: status, message:msg});
			}else{
