...
exports.associateUserToFolder = function(email, folder, cbk) {
  console.log('associateUserToFolder', email, folder);
  ...
  db.serialize(function() {
    ...
    db.each("SELECT * FROM open_pages WHERE folder=?", folder, function(err, row) {
        console.log('associateUserToFolder - association exists', err, folder, row);
        if(err) {
          cbk(err);
        }
      ...
      if (err) {
        ...
        db.each("SELECT * FROM open_pages WHERE email=?", email, function(err, row) {
          if(err) {
            ...
            // update
            console.log('associateUserToFolder - user exists', email, folder);
            db.run('UPDATE INTO open_pages VALUES (?,?) WHERE email=?', email, folder, email);
          }
        }, function(err, numRows) {
          if (err) {
            cbk(err);
            ...
            // cerate the record
            console.log('associateUserToFolder - association DOES NOT exist', email, folder);
            db.run('INSERT INTO open_pages VALUES (?,?)', email, folder);
          }
          else {
            cbk();
          }
