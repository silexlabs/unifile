    ...
    var postReq = https.request(postOptions, function(res) {
      ...
      res.on('data', function (buff) {
        ...
        if(data.email && data.status === 'okay') {
          ...
          request.session.open_pages_user.folder_name = shasum.digest('hex');
...
exports.getRootPath = function(request, path) {
  var resolvedPath = pathModule.resolve(exports.config.ROOT + '/' + request.session.open_pages_user.folder_name + '/' + path);
  return resolvedPath;
}
