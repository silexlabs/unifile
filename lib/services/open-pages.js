...
exports.getInfo = function (request) {
  ...
  return {
    ...
    visible: exports.config.ENABLED === true, // true if it should be listed in /v1.0/services/list/
...
exports.init = function (app, express, options) {
  ...
  if (exports.config.ENABLED !== true) {
    return;
  }
  // form for local auth
  ...
  // users folder online
  app.get(exports.config.OPEN_PAGES_WEB_ROUTE + '/:folder/:path', function(request, response, next) {
      ...
      db.all("SELECT * FROM open_pages WHERE folder=? LIMIT 1", folder, function(err, rows) {
          db.close();
          if(err) {
            console.error('error in DB: ', err);
            response.status(500).json({ error: 'Nothing here'  })
          }
          else if(rows.length == 0) {
            response.status(404).json({ error: 'Nothing here'  })
          }
          else {
            var row = rows[0];
            var resolvedPath = pathModule.resolve(exports.config.ROOT, row.email, path);
