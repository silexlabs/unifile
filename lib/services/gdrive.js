...
exports.mkdir = function (path, request, cbk) {
	if (!request.session.gdrive_token){
		...
		if (path_arr.length>1){
			...
			search_folder_recursive(path_arr, null, request, function (parent_folder) {
				if (parent_folder){
					console.log("parent_folder found "+parent_folder.title);
					console.log("List parent_folder with id "+parent_folder.id);
					...
					createFile(parent_folder, folder, request, cbk);
				}
				else{
					cbk ({success:false});
				}
 ...
 */
function createFile (parent_folder, file, request, cbk) {
	...
	.execute(function(err, client) {
		...
		try{
			client
			.drive.files.insert({'resource': file})
			.withAuthClient(oauth2Client)
...
exports.cp = function (src, dst, request, cbk) {
	if (!request.session.gdrive_token){
		...
		if (src_path_arr.length>1 && dst_path_arr.length>1){
			...
			search_folder_recursive(src_path_arr, null, request, function (src_file) {
				if (src_file){
					console.log("src_file found "+src_file.title);
					console.log("List src_file with id "+src_file.id);

					search_folder_recursive(dst_path_arr, null, request, function (dst_parent_folder) {
...
function copyFile (src_file, dst_file_data, request, cbk) {
	...
	.execute(function(err, client) {
		...
		try{
			...
			console.error("Error while deleting file "+src_file.title+". Error: "+e);
		}
	});
}
