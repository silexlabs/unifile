 ...
 */
exports.getOAuth2Client = function (request) {
	if (exports.oAuth2Client) return exports.oAuth2Client;
	var cbk_url = 'http://'+request.headers.host+config.auth_url_callback;
	exports.oAuth2Client = new googleapis.OAuth2Client(config.client_id, config.client_secret, cbk_url);
	return exports.oAuth2Client;
	// return new googleapis.OAuth2Client(config.client_id, config.client_secret, config.redirect_uris[0]);
}
/**
 * Connect to the service, i.e. ask for a request token.
 * The request token is required so that the user can allow our app to access his data.
...
// generates a url allows offline access and asks permissions
	exports.logout(request, response, next, function () {
		var url = exports.getOAuth2Client(request).generateAuthUrl({
...
exports.login = function (request, response, next, cbk) {
	if (!request.session.gdrive_request_token){
		...
		if (request.session.gdrive_account){
			...
			exports.getOAuth2Client(request).credentials = request.session.gdrive_token;
		...
		else exports.getOAuth2Client(request).getToken(request.session.gdrive_request_token, function(err, tokens) {
...
function getAccount (request, response, next, cbk) {
	...
	.execute(function(err, client) {
		...
		client
		.drive.about.get()
		.withAuthClient(exports.getOAuth2Client(request))
 ...
 */
 function search_folder_recursive(path_arr, parent_id, request, response, next, cbk) {
	search_folder(path_arr.shift(), parent_id, request, response, next, function (folder, err) {
		if (folder){
...
console.log('createFile', file, data);
	var dataObj ={};
	...
	.execute(function(err, client) {
		...
		try{
			client
			.drive.files.insert({'body': file}, dataObj)
			.withAuthClient(exports.getOAuth2Client(request))
...
exports.put = function (path, data, request, response, next, cbk) {
	if (!request.session.gdrive_token){
		...
		if (path_arr.length>1){
			...
			if (path_arr[path_arr.length-1]=="") path_arr.pop();
			// get the last item and assume it is the name of the folder to create
			var dst_file_name = path_arr.pop();
			console.log('file name ', dst_file_name, path_arr)
			...
			search_folder_recursive(path_arr, null, request, response, next, function (parent_folder, err) {
				if (parent_folder){
					...
					var dst_file = new Object();
					dst_file.title = dst_file_name;
					dst_file.parents = [{"id":parent_folder.id}];

					// create the empty file
					createFile(parent_folder, dst_file, data, request, response, next, cbk);
				}
				else{
					var errCode = null;
					if (err && err.code){
...
exports.get = function (path, request, response, next, cbk) {
	...
	if (!request.session.gdrive_token){
		...
		if (path_arr.length>2){
			...
			// remove the last empty item
			if (path_arr[path_arr.length-1]=="") path_arr.pop();
			console.log('get search', path_arr);

			search_folder_recursive(path_arr, null, request, response, next, function (file, err) {
				console.log('get found', file.title, file.kind);
				if (file){
			...
			console.log('get search in root', path_arr);
			listFiles(request.session.gdrive_account.rootFolderId, request, response, next, cbk);
		}
	}
}
function getFile (file, request, response, next, cbk) {
	console.log('getFile', file.title);
	exports.getOAuth2Client(request).credentials = request.session.gdrive_token;
	googleapis
	.discover('drive', 'v2')
	.execute(function(err, client) {
...
function downloadFile(file, accessToken, callback) {
	console.log('downloadFile', file.title);
  if (file.downloadUrl) {
	...
	console.log('downloadFile https.get', options);
	...
	console.log('downloadFile https.get end', store);
				callback(store);
