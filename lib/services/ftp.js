...
exports.init = function (app, express, options) {
  ...
  app.get(exports.config.AUTH_FORM_ROUTE, function(request, response, next){
    utils.displayLoginForm(200, response, options.ftp.LOGIN_TEMPLATE_PATH, options.ftp.LOGIN_CSS_PATH, {});
  });
  ...
  app.post(exports.config.AUTH_FORM_SUBMIT_ROUTE, function(request, response, next){
    if (request.param('password') && request.param('username') && request.param('host') && request.param('port')){
      exports.ftpLogin(request, request.param('password'), request.param('username'), request.param('host'), request.param('port'),
        function () {
          utils.displayLoginForm(200, response, options.ftp.LOGIN_TEMPLATE_PATH, options.ftp.LOGIN_CSS_PATH, {success: true});
        },
        function (err) {
          // stop listening
          // make app crash
...
exports.reconnect = function (request, cbk) {
  var onError = function (opt_err) {
    opt_err = opt_err || '';
    console.error('could not reconnect', request.session.ftp_user, opt_err);
    cbk(opt_err);
  }
  if (exports.isLoggedIn(request)){
    cbk();
  }
  else if (request.session.ftp_user){
...
exports.rm = function (path, request, response, next, cbk) {
  exports.reconnect(request, function (err) {
    if (err){
      ...
      path = path.replace(/\/+$/,''); // remove last slash on directories
      var parentDir = path.replace(/^(.*)\/[^\/]+$/,'$1');
        ...
        if (!err) {
          ...
          ftpClientsArray[request.session.ftpClientId].put(tmpFileName, dst, function(err) {
            try{
              ...
              console.error('could not unlink file (ftp::cp)', err);
            }
          });
        }
        else{
    ...
    if (err){
      ...
      ftpClientsArray[request.session.ftpClientId].get(path, function(err, stream) {
        if (!err && stream) {
          ...
          stream.once('close', function() {
            ...
            setTimeout(function(){
              try{
                ...
                console.error('could not unlink file (ftp::get)', err);
              }
            }, 10000);
          });
          ...
          if (!err) err = {};
          cbk({success:false, message: err.message});
        }
      });
    }
  });
