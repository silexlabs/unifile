...
exports.ls = function (path, request, response, next, cbk) {
  ...
  ftpClient.list(path, function(err, filesArray) {
    if (!err){
      var newPath = path;
      var filesData = [];
      for(var idx=0; idx<filesArray.length; idx++){
        ...
        if (file.name && file.name != ''){
          console.log('got list of files', file);
          //console.dir(file);
          filesData.push({
            bytes: file.size,
            modified: file.date,
...
exports.rm = function (path, request, response, next, cbk) {
  ...
  ftpClient.delete(path, function (err) {
    if (err){
      console.log(err);
      cbk({success:false, message: err});
    }
...
exports.mkdir = function (path, request, response, next, cbk) {
  ...
  ftpClient.mkdir(path, function (err) {
    if (!err){
      cbk({success:true});
    }
    else{
 ...
 */
 exports.cp = function (src, dst, request, response, next, cbk) {
  ...
  ftpClient.get(src, function(err, stream) {
    if (!err) {
