...
function removeFile(path, treeRes) {
	...
	if(filteredTree.length === treeRes.tree.length) throw new Error('Not Found');
...
class GitHubConnector {
			...
			return new Promise((resolve, reject) => {
				request({
					...
					if(err) reject('Error while calling GitHub API. ' + err);
						...
						reject('Unable to get access token. Please check your credentials.');
	...
	setAccessToken(session, token) {
		...
		if(!token.startsWith('token ') && !token.startsWith('Basic '))
			return Promise.reject('Invalid token. It must start with either "token" or "Basic".');
	...
	createWriteStream(session, path) {
		...
		if(splitPath.length < 3) {
			...
			process.nextTick(() => {
				stream.emit('error', new Error('This folder can only contain folders. Files can go in sub-folders.'));
			});
			return stream;
		}
		...
		if(isStream) return promise;
			...
			return promise.then(function(res) {
				if(res.type === 'file') {
					...
					return Promise.reject(new Error('Path is a directory'));
				}
			});
		}
	}
	...
	rename(session, src, dest) {
		...
		switch (splitPath.length) {
			case 0: // Error
				return Promise.reject('Cannot rename path with an empty name.');
			case 1: // Rename repo
				apiPath = '/repos/' + session.account.login + '/' + splitPath[0];
	...
	unlink(session, path) {
		if(!path) return Promise.reject('Cannot remove path with an empty name.');
		const splitPath = getPathTokens(path);
		if(splitPath.length < 3) return Promise.reject('Path is a folder. Use rmdir()');
	...
	rmdir(session, path) {
		...
		switch (splitPath.length) {
			case 0: // Error
				return Promise.reject('Cannot remove path with an empty name.');
			case 1: // Remove repo
	...
	batch(session, actions, message) {
		...
		while(actionQueue.length > 0) {
			...
			switch (splitPath.length) {
				case 0: return Promise.reject(new BatchError('Cannot execute batch action without a path'));
				case 1:
				case 2:
					...
					switch (actionName) {
						...
						case 'mkdir':
							...
							break;
						case 'writefile':
							return Promise.reject(new Error(`Could not complete action ${actionName}: Cannot create file here.`));
						case 'rename':
							if(!action.destination)
								return new Promise.reject(new BatchError('Rename actions should have a destination'));
							actionsChain = actionsChain.then(() => this.rename(session, action.path, action.destination))
							.catch((err) => err.name !== 'BatchError',
								(err) => {throw new BatchError(`Could not complete action ${actionName}: ${err.message}`);});
							break;
						default:
				...
				default:
							...
							switch (currentAction.name.toLowerCase()) {
								...
								case 'rename':
									if(!currentAction.destination)
										return new Promise.reject(new Error('Rename actions should have a destination'));
									const src = path;
								...
								case 'writefile':
									if(!currentAction.content)
										return new Promise.reject(new Error('WriteFile actions should have a content'));
									treeRes.tree.push({
										path: path,
					...
					.catch((err) => err.name !== 'BatchError', (err) => {
						throw new BatchError(`Could not modify tree: ${err.message}`);
					});
			}
		}
		return actionsChain;
