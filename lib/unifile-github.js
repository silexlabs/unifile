...
class GitHubConnector {
  ...
  getAuthorizeURL() {
    return Promise.resolve(GH_OAUTH_URL + '/authorize?' + APP_PERMISSION + '&client_id=' + this.config.clientId + '&state=' + this.config.state);
  }
    ...
    if (!session.token)
      ...
      return transformTree(session, splitPath[0], function(treeRes){
        for(let i=0; i<actions.length; i++){
          ...
          let path = splitPath.slice(2).join('/');
          switch(actions[i].name.toLowerCase()){
            case 'unlink':
            case 'rmdir':
            ...
            case 'mkdir':
              ...
              treeRes.tree.push({path: path, mode: '040000', type: 'tree'});
              break;
            case 'writeFile':
