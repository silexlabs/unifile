...
class GitHubConnector {
  ...
  createWriteStream(path){
    ...
    var splitPath = getPathTokens(path);
    var apiPath = '/repos/' + this.session.account.login + '/' + splitPath[0] + '/git/blobs';
    var stream = streamAPI(this.session.token, apiPath, {}, 'POST');
  ...
  readFile(path) {
    if (!this.isConnected){
      ...
      var apiPath = '/repos/' + this.session.account.login
  ...
  createReadStream(path){
    ...
    return streamAPI(this.session.token, apiPath, {ref: splitPath[1]}, 'GET');
  }
    ...
    if (!this.isConnected){
      ...
      switch (splitPath.length) {
        ...
        case 1: // Rename repo
          var apiPath = '/repos/' + this.session.account.login + '/' + splitPath[0];
          reqData = {name: dest};
          return callAPI(this.session.token, apiPath, reqData, 'PATCH');
          break;
        case 2: // Rename branch (actually copy src to dest then remove src)
          ...
          return createBranch(request.session, splitPath[0], splitPathDest[1], splitPath[1])
          .then(function() {
            return callAPI(this.session.token, apiPath + splitPath[1], null, 'DELETE');
          });
          break;
        default: // Rename a file/folder
          return move(this.session, splitPath[0], splitPath.slice(2).join('/'), splitPathDest.slice(2).join('/'), splitPath[1]);
      }
    }
  }
  ...
  unlink(path) {
    if (!this.isConnected){
      ...
      var splitPath = getPathTokens(path);
      var repoPath = '/repos/' + this.session.account.login + '/' + splitPath[0];
      switch (splitPath.length) {
        case 0: // Error
          return Promise.reject('Cannot remove dir with an empty name.');
        case 1: // Remove repo
          return callAPI(this.session.token, repoPath, null, 'DELETE');
        case 2: // Remove branch
          return callAPI(this.session.token, repoPath + '/branches', null, 'GET')
          .then(branches => {
            if(branches.length > 1)
              return callAPI(this.session.token, repoPath + '/git/refs/heads/' + splitPath[1], null, 'DELETE');
            else{
        ...
        default: // Remove file/folder
          return removeFile(this.session, splitPath[0], splitPath.slice(2).join('/'), splitPath[1]);
      }
    }
  }
}
