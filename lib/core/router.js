 ...
 */
exports.route = function (serviceName, url_arr, request, response, next, sendResponse) {
				...
				switch (url_arr[0]){
					case "exec":
						if (url_arr.length > 2){
							...
							return exec(service, command, path, request, response, next, sendResponse);
						}
						break;
					case "connect":
...
function exec (service, command, path, request, response, next, sendResponse) {
	switch (command){
		case "ls":
			...
			service.get(path.normalize(),
				...
				function(status, text_content, mime_type, path){
					if (text_content || path){
						sendResponse(response, status, text_content, mime_type, path);
					}
					else if (status){
						sendResponse(response, status, status);
					}
      ...
      if (request.files && request.files.data){
        ...
        var doPut = function (data) {
          if(!data || !data.length){
            ...
            sendResponse(response, {success:false, message:'File upload failed: no data provided in GET nor POST.'});
          }
          else {
            // upload success
            service.put(path.normalize(), data,
	...
	sendResponse(response, {success:false, message:'Nothing here ('+service+', '+command+', '+path+'). Returns a list of routes.'},
