...
Header add Access-Control-Allow-Headers "X-Haxe-Remoting"
Header add Access-Control-Request-Method "*"
...
function exec (service, command, path, request, response) {
	...
	switch (command){
		...
		case "get":
			service.get(path, 
				request, response, 
				function(status, text_content){
					if (text_content){
						sendResponse(response, text_content);
					}
					else if (status){
						sendResponse(response, {status:status});
					}
