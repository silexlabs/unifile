...
describe.only('GitHubConnector', function() {
	...
	const authConfig = Object.assign({}, defaultConfig, {clientSecret: process.env.GITHUB_SECRET});

	before('Init session with a valid account and tests repo', function() {
		if(isEnvValid()) {
			return new GitHubConnector(authConfig).setAccessToken(session, process.env.GITHUB_TOKEN);
		}
	});

	beforeEach('Slows down test to avoid collision', function(done) {
		setTimeout(done, 200);
	});
		...
		it('overrides infos when given in config', function() {
			...
			Object.keys(overrides).forEach((key) => {
				...
				if(key === 'name')
					...
					expect(connector.name).to.equal(githubDefaultInfos.name);
			});
		});

		it('ignores invalid infos', function() {
			const connector = new GitHubConnector({infos: 3, redirectUri: 'a', clientId: 'a', clientSecret: 'a'});
			expect(connector.infos).to.deep.equal(githubDefaultInfos);
		});
	});
	...
	describe('getInfos()', function() {
		...
		beforeEach('Instanciation', function() {
			connector = new GitHubConnector(defaultConfig);
		});

		it('returns an infos object', function() {
			const infos = connector.getInfos({});
			expect(infos).to.be.an.instanceof(Object);
			expect(infos.name).to.equal(githubDefaultInfos.name);
			expect(infos.displayName).to.equal(githubDefaultInfos.displayName);
			expect(infos.icon).to.equal(githubDefaultInfos.icon);
			expect(infos.description).to.equal(githubDefaultInfos.description);
		...
		it('returns a customed infos object is told so', function() {
			const infos = new GitHubConnector(Object.assign({}, defaultConfig, {infos: {icon: 'ooo'}, redirectUri: 'a'}))
			.getInfos({});
			expect(infos).to.be.an.instanceof(Object);
			expect(infos.name).to.equal(githubDefaultInfos.name);
			expect(infos.displayName).to.equal(githubDefaultInfos.displayName);
		...
		it('uses session account when available', function() {
			if(!isEnvValid()) this.skip();
				...
				const infos = new GitHubConnector(authConfig).getInfos(session);
				expect(infos).to.be.an.instanceof(Object);
				expect(infos.name).to.equal(githubDefaultInfos.name);
				expect(infos.displayName).to.equal(githubDefaultInfos.displayName);
				expect(infos.icon).to.equal('../assets/github.png');
				expect(infos.description).to.equal(githubDefaultInfos.description);
	...
	describe('getAuthorizeURL()', function() {
		...
		it('returns a promise for the authorization URL', function() {
			...
			.then((url) => {
				...
				expect(authUrl.host).to.equal('github.com');
				expect(authUrl.pathname).to.equal('/login/oauth/authorize');
				expect(authUrl.query.scope).to.equal('repo,delete_repo,user');
				expect(authUrl.query.client_id).to.equal(defaultConfig.clientId);
				expect(authUrl.query.state).to.exist;
				expect(authUrl.query.redirect_uri).to.be.undefined;
			});
		});

		it('returns a promise for the authorization URL with a custom redirection', function() {
			return new GitHubConnector(Object.assign({}, defaultConfig, {redirectUri: 'a'})).getAuthorizeURL({})
			.then((url) => {
				const authUrl = Url.parse(url, true);
				expect(authUrl.protocol).to.equal('https:');
				expect(authUrl.host).to.equal('github.com');
				...
				expect(authUrl.query.scope).to.equal('repo,delete_repo,user');
				expect(authUrl.query.client_id).to.equal(defaultConfig.clientId);
				expect(authUrl.query.redirect_uri).to.equal('a');
				expect(authUrl.query.state).to.exist;
			});
		});

		it('returns a promise for the authorization URL with a custom service host', function() {
			return new GitHubConnector(Object.assign({}, defaultConfig, {serviceHost: 'mygit.org'})).getAuthorizeURL({})
			.then((url) => {
				const authUrl = Url.parse(url, true);
				expect(authUrl.protocol).to.equal('https:');
				expect(authUrl.host).to.equal('mygit.org');
				expect(authUrl.pathname).to.equal('/login/oauth/authorize');
				expect(authUrl.query.scope).to.equal('repo,delete_repo,user');
				expect(authUrl.query.client_id).to.equal(defaultConfig.clientId);
				expect(authUrl.query.redirect_uri).to.be.undefined;
	...
	describe('login()', function() {
		...
		let connector;
		beforeEach('Instanciation', function() {
			connector = new GitHubConnector(authConfig);
		});
		...
		it('rejects the promise with the wrong credentials', function() {
			return expect(connector.login({}, 'https://toto:roro@github.com')).to.be.rejectedWith('credentials');
		});
		...
		it('accepts a string as login infos', function() {
			...
			const session = {};
			return connector.login(session, `https://${user}:${pwd}@github.com`)
			.then(() => {
				checkSession(session);
			});
		});
	...
	describe('readdir()', function() {
		...
		before('Init tests repo', function() {
			if(isEnvValid()) {
				connector = new GitHubConnector(authConfig);
				return connector.setAccessToken(session, process.env.GITHUB_TOKEN)
